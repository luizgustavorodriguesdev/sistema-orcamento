...
const handleSearch = () => {
    router.get(route('themes.index'), { search: searchQuery.value }, {
        preserveState: true,
        replace: true,
    });
};
...
const submitForm = () => {
    if (isEditing.value) {
        form.put(route('themes.update', currentId.value), {
            onSuccess: () => {
...
const deleteTheme = (id) => {
    if (confirm('Tem certeza que deseja excluir este tema? Isto removerá a associação em todos os produtos.')) {
        router.delete(route('themes.destroy', id), {
            preserveScroll: true
        });
    }
};
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Gestão de Temas</h2>
        </template>
                    ...
                    <div class="p-6 text-gray-900">
                        ...
                        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table class="w-full text-sm text-left text-gray-500">
                                ...
                                <tbody>
                                    <tr v-for="theme in themes.data" :key="theme.id" class="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-center whitespace-nowrap">
                                            <button @click="openEditModal(theme)" class="font-medium text-blue-600 hover:underline mr-4">Editar</button>
                                            <button @click="deleteTheme(theme.id)" class="font-medium text-red-600 hover:underline">Deletar</button>
                                        </td>
                                    </tr>
                        ...
                        <!-- Pagination Links -->
                        <div v-if="themes.links && themes.links.length > 3" class="mt-6 flex justify-center">
                            <div class="flex">
                                <template v-for="(link, key) in themes.links" :key="key">
                                    <Link
                                        v-if="link.url"
                                        :href="link.url"
                                        v-html="link.label"
        ...
        <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 space-y-4">
                <div class="flex justify-between items-center border-b pb-2">
                    <h3 class="text-lg font-bold text-gray-900">{{ isEditing ? 'Editar Tema' : 'Novo Tema' }}</h3>
                    <button @click="isModalOpen = false" class="text-gray-400 hover:text-gray-600 text-xl">&times;</button>
                </div>

                <form @submit.prevent="submitForm" class="space-y-4">
                    <div>
                        <label for="name" class="block font-medium text-sm text-gray-700">Nome do Tema</label>
                        <input
                            id="name"
                            type="text"
                            v-model="form.name"
                            required
                            autofocus
                            placeholder="Ex: Dia das Mães, Casamento"
