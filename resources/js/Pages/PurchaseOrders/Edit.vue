...
const form = useForm({
    ...
    status: props.order.status, // Rascunho, Aprovado, Cancelado
    items: props.order.items.map(item => ({
        product_id: item.product_id,
        name: item.product?.name || 'Produto Excluído',
...
<template>
    <Head title="Editar Ordem de Compra" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center w-full">
                <div>
                    <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Editar Ordem de Compra</h2>
                    <p class="text-xs text-slate-400 mt-1">Atualize os detalhes do pedido de reabastecimento.</p>
                </div>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-sm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div class="md:col-span-1">
                                    <label for="status" class="block text-xxs font-black text-slate-400 uppercase mb-1">Status do Pedido</label>
                                    <select 
                                        id="status" 
                                        v-model="form.status" 
                                        ...
                                        required
                                    >
                                        <option value="Rascunho">Rascunho (Edição)</option>
                                        <option value="Aprovado">Aprovado (Aguardando Entrada)</option>
                                        <option value="Cancelado">Cancelado</option>
                                    </select>
                                    <p v-if="form.errors.status" class="text-xs text-rose-500 mt-1">{{ form.errors.status }}</p>
                                </div>
                                <div>
                        ...
                        <!-- Adicionar Produto à Lista (apenas se for Rascunho) -->
                        <div v-if="order.status === 'Rascunho'" class="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-sm">
                            ...
                            <table class="w-full text-xs text-left text-slate-300">
                                <thead class="text-xxs text-slate-400 uppercase bg-slate-950 border-b border-slate-800">
                                    <tr>
                                        ...
                                        <th v-if="order.status === 'Rascunho'" scope="col" class="px-6 py-3 font-black text-center">Ações</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(item, index) in form.items" :key="index" class="border-b border-slate-800/50 hover:bg-slate-800/20 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-center font-mono">
                                            ...
                                            />
                                            <span v-else class="text-slate-300 font-mono">{{ item.quantity }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-right font-mono">
                                            ...
                                            />
                                            <span v-else class="text-slate-300 font-mono">{{ formatCurrency(item.price) }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-right font-mono text-white font-semibold">{{ formatCurrency(item.quantity * item.price) }}</td>
                                        <td v-if="order.status === 'Rascunho'" class="px-6 py-4 text-center">
