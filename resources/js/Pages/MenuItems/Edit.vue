...
const form = useForm({
    ...
    product_id: props.menuItem.product_id || '',
    order: props.menuItem.order,
    is_active: props.menuItem.is_active === 1 || props.menuItem.is_active === true,
});
...
<template>
    <Head title="Editar Item de Menu" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar Item de Menu</h2>
        </template>
                        ...
                        <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <div class="flex items-center justify-end space-x-4 border-t pt-4">
                                ...
                                <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                                    {{ form.processing ? 'A Atualizar...' : 'Atualizar Item de Menu' }}
                                </button>
                            </div>
                        </form>
