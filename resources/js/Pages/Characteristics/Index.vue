...
const handleSearch = () => {
    router.get(route('characteristics.index'), { search: searchQuery.value }, {
        preserveState: true,
        replace: true,
    });
};
...
const submitForm = () => {
    if (isEditing.value) {
        form.put(route('characteristics.update', currentId.value), {
            onSuccess: () => {
        ...
        form.post(route('characteristics.store'), {
            onSuccess: () => {
                isModalOpen.value = false;
                form.reset();
            }
...
const deleteChar = (id) => {
    if (confirm('Tem certeza que deseja excluir esta característica? Isto removerá a associação em todos os produtos.')) {
        router.delete(route('characteristics.destroy', id), {
            preserveScroll: true
        });
    }
};
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Gestão de Características</h2>
        </template>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        ...
                        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
                            <h3 class="text-2xl font-bold">Lista de Características</h3>
                        ...
                        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table class="w-full text-sm text-left text-gray-500">
                                ...
                                <tbody>
                                    <tr v-for="char in characteristics.data" :key="char.id" class="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-center whitespace-nowrap">
                                            <button @click="openEditModal(char)" class="font-medium text-blue-600 hover:underline mr-4">Editar</button>
                                            <button @click="deleteChar(char.id)" class="font-medium text-red-600 hover:underline">Deletar</button>
                                        </td>
                                    </tr>
                                    <tr v-if="characteristics.data.length === 0">
                        ...
                        <!-- Pagination Links -->
                        <div v-if="characteristics.links && characteristics.links.length > 3" class="mt-6 flex justify-center">
                            <div class="flex">
                                <template v-for="(link, key) in characteristics.links" :key="key">
                                    <Link
                                        v-if="link.url"
                                        :href="link.url"
                                        v-html="link.label"
        ...
        <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 space-y-4">
                <div class="flex justify-between items-center border-b pb-2">
                    <h3 class="text-lg font-bold text-gray-900">{{ isEditing ? 'Editar Característica' : 'Nova Característica' }}</h3>
                    <button @click="isModalOpen = false" class="text-gray-400 hover:text-gray-600 text-xl">&times;</button>
                </div>

                <form @submit.prevent="submitForm" class="space-y-4">
                    <div>
                        <label for="name" class="block font-medium text-sm text-gray-700">Nome da Característica</label>
                        <input
                            id="name"
                            type="text"
                            v-model="form.name"
                            required
                            autofocus
                            placeholder="Ex: Livre de BPA, Parede Dupla"
